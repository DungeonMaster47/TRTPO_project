...
def get_group_schedule(schedule):
    results = ''
...
def get_current_group_schedules(group):
    ...
    result = 'Расписание на сегодня \n' + \
             get_group_schedule(data["todaySchedules"]) + '\n' + \
             '\nРасписание на завтра' + '\n' + \
             get_group_schedule(data["tomorrowSchedules"])
    return result


def get_employee_schedule(schedule):
    results = ''
    for lesson in schedule:
        auditory = ''
        if len(lesson['auditory']) > 0:
...
def get_current_employee_schedules(name):
    name = name.split(sep=' ')
    if len(name) < 3:
    ...
    result = 'Расписание на сегодня \n' + \
             get_employee_schedule(schedule["todaySchedules"]) + '\n' + \
             '\nРасписание на завтра' + '\n' + \
             get_employee_schedule(schedule["tomorrowSchedules"])
    return result
